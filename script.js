/** 버전: 6.0 | 최종 수정일: 2025-11-04 (데이터 복원 로직 대폭 강화 및 이벤트 핸들링 수정) */
...
todayTbody.addEventListener("click",e=>{if(e.target.classList.contains("location-clickable")){const centerName=e.target.dataset.centerName;copyAddressToClipboard(centerName)}});
addressDisplay.addEventListener("click",e=>{if(e.target.classList.contains("address-clickable")){copyTextToClipboard(e.target.dataset.address,"주소가 복사되었습니다.")}});
batchApplyBtn.addEventListener("click",()=>{const from="direct"===batchFromSelect.value?batchFromCustom.value:batchFromSelect.value,to="direct"===batchToSelect.value?batchToCustom.value:batchToSelect.value,income=parseFloat(batchIncomeInput.value)||0;if(from&&to&&0<income){let records=JSON.parse(localStorage.getItem("records"))||[];let updatedCount=0;const recordsToUpdate=records.filter(r=>"화물운송"===r.type&&r.from===from&&r.to===to&&0===r.income);0===recordsToUpdate.length?showToast("해당 구간의 미정산 기록이 없습니다."):confirm(`정말로 '${from} -> ${to}' 구간의 미정산 기록 ${recordsToUpdate.length}건에 운임 ${income}만원을 일괄 적용하시겠습니까?`)&&(records=records.map(r=>"화물운송"===r.type&&r.from===from&&r.to===to&&0===r.income?(updatedCount++,{...r,income:1e4*income}):r),localStorage.setItem("records",JSON.stringify(records)),batchStatus.textContent=`✅ ${updatedCount}건의 운임이 성공적으로 적용되었습니다!`,batchFromSelect.value=getCenters()[0],batchToSelect.value=getCenters()[0],batchIncomeInput.value="",updateAllDisplays(),setTimeout((()=>batchStatus.textContent=""),3e3))}else alert("출발지, 도착지를 선택하고 유효한 운송 수입을 입력하세요.")});
subsidySaveBtn.addEventListener("click",()=>{const limit=subsidyLimitInput.value;localStorage.setItem("fuel_subsidy_limit",limit);showToast(`보조금 한도가 ${limit}L로 저장되었습니다.`)});
mileageCorrectionSaveBtn.addEventListener("click",()=>{const correction=mileageCorrectionInput.value;localStorage.setItem("mileage_correction",correction);showToast(`주행거리 보정값이 ${correction} km로 저장되었습니다.`);displayCumulativeData()});
exportCsvBtn.addEventListener("click",exportToCsv);
exportJsonBtn.addEventListener("click",exportToJson);
importJsonBtn.addEventListener("click",()=>importFileInput.click());
importFileInput.addEventListener("change",importFromJson);
clearBtn.addEventListener("click",()=>{confirm("정말로 모든 기록과 설정을 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.")&&(localStorage.clear(),alert("모든 데이터가 삭제되었습니다."),location.reload())});
tabBtns.forEach(btn=>{btn.addEventListener("click",event=>{event.preventDefault();tabBtns.forEach(b=>b.classList.remove("active"));btn.classList.add("active");viewContents.forEach(c=>c.classList.remove("active"));document.getElementById(btn.dataset.view+"-view").classList.add("active");updateAllDisplays()})});
...
prevDayBtn.addEventListener("click",()=>{const currentDate=new Date(todayDatePicker.value);currentDate.setDate(currentDate.getDate()-1);todayDatePicker.value=formatDate(currentDate);displayTodayRecords()});
nextDayBtn.addEventListener("click",()=>{const currentDate=new Date(todayDatePicker.value);currentDate.setDate(currentDate.getDate()+1);todayDatePicker.value=formatDate(currentDate);displayTodayRecords()});
...
function calculateLiters(){const costInManwon=parseFloat(costInput.value)||0,type=typeSelect.value;if(document.activeElement===costInput){if("주유소"===type){const unitPrice=parseFloat(fuelUnitPriceInput.value)||0;0<costInManwon&&0<unitPrice&&(fuelLitersInput.value=(1e4*costInManwon/unitPrice).toFixed(2))}else if("요소수"===type){const unitPrice=parseFloat(ureaUnitPriceInput.value)||0;0<costInManwon&&0<unitPrice&&(ureaLitersInput.value=(1e4*costInManwon/unitPrice).toFixed(2))}}}
...
fromSelect.addEventListener("change",()=>{fromCustom.classList.toggle("hidden","direct"!==fromSelect.value);autoFillIncome();updateAddressDisplay()});
toSelect.addEventListener("change",()=>{toCustom.classList.toggle("hidden","direct"!==toSelect.value);autoFillIncome();updateAddressDisplay()});
batchFromSelect.addEventListener("change",()=>batchFromCustom.classList.toggle("hidden","direct"!==batchFromSelect.value));
batchToSelect.addEventListener("change",()=>batchToCustom.classList.toggle("hidden","direct"!==batchToSelect.value));
cancelEditBtn.addEventListener("click",cancelEdit);
function autoFillIncome(){if("화물운송"===typeSelect.value){const from=fromSelect.value,to=toSelect.value;if(from&&to&&"direct"!==from&&"direct"!==to){const fareKey=`${from}-${to}`,fares=JSON.parse(localStorage.getItem("saved_fares"))||{};fares[fareKey]&&(incomeInput.value=(fares[fareKey]/1e4).toFixed(2))}}}
goToSettingsBtn.addEventListener("click",()=>{mainPage.classList.add("hidden");settingsPage.classList.remove("hidden");goToSettingsBtn.classList.add("hidden");backToMainBtn.classList.remove("hidden");displayCenterList();mileageCorrectionInput.value=localStorage.getItem("mileage_correction")||"0"});
backToMainBtn.addEventListener("click",()=>{mainPage.classList.remove("hidden");settingsPage.classList.add("hidden");goToSettingsBtn.classList.remove("hidden");backToMainBtn.classList.add("hidden")});
addCenterBtn.addEventListener("click",()=>{const newName=newCenterNameInput.value,newAddress=newCenterAddressInput.value,newMemo=newCenterMemoInput.value;addCenter(newName,newAddress,newMemo)?(newCenterNameInput.value="",newCenterAddressInput.value="",newCenterMemoInput.value=""):alert("지역 이름을 입력하거나, 이미 존재하지 않는 이름을 사용해주세요.")});
centerListContainer.addEventListener("click",e=>{e.target.classList.contains("delete-btn")&&deleteCenter(e.target.closest(".center-item").dataset.centerName);e.target.classList.contains("edit-btn")&&handleCenterEdit(e)});
[toggleCenterManagementBtn,toggleBatchApplyBtn,toggleSubsidyManagementBtn,toggleMileageManagementBtn,toggleDataManagementBtn].forEach(header=>{header&&header.addEventListener("click",()=>{const body=header.nextElementSibling;body.classList.toggle("hidden");header.classList.toggle("active")})});
function initialSetup(){updateCentersFromRecords();populateCenterSelectors();populateSelectors();cancelEdit();todayDatePicker.value=getTodayString();updateAllDisplays()}
